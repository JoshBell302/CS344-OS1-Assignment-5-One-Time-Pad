...
int main(int argc, char* argv[]) {
	// Setting Variables
	int connectionSocket, charsRead, keyStart, textCharsInt, keyCharsInt, decCharsInt;
	...
	char* decryption = calloc((BUFFER_LENGTH)+1, sizeof(char));
	int decLength = 0;
	int a = 0;
	int left = 0;
	...
	case 0:
		...
		while (1) {
			...
			for (int i = 0; i < textLength; i++) {
				...
				decCharsInt = textCharsInt - keyCharsInt;

				// If the value is a negative value need to add 26 to create the correct char
				if (decCharsInt < 0)
					decCharsInt = decCharsInt + AVAILABLE_VALUE;

				decCharsInt = decCharsInt % AVAILABLE_VALUE;

				// Changes to space if its not a member of the alphabet
				if (decCharsInt < 0 || decCharsInt > 25)
					decryption[i] = ' ';

				// Fill the char into the encrypt string
				else
					decryption[i] = decCharsInt + ASCII_APLHABET;
			}
